...
# Change path if user isn't in GidiJobs root
while [ ! -f "${CATS_ROOT_PATH_CHECK}" ] && [ `pwd` != "/" ]; do
    echo -n "GidiJobs not found in `pwd`... trying "
    cd ..
    echo -n "`pwd`"
...
then
    echo "Cannot find GidiJobs. Please run this script "
    echo "from a path within a GidiJobs installation."
    exit 1
fi
