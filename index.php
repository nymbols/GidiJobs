...
/* Hack to make GidiJobs work with E_STRICT. */
...
{
    die('GidiJobs Error: session.auto_start must be set to 0 in php.ini.');
}

/* Proper extensions loaded?! */
if (!function_exists('mysql_connect') || !function_exists('session_start'))
{
    die('GidiJobs Error: All required PHP extensions are not loaded.');
}
...
else
{
    if ($_GET['m'] == 'logout')
    {
        ...
        {
            CATSUtility::transferURL('http://www.gidijobs.co');
        }
        else
        {
            CATSUtility::transferRelativeURI($URI);
