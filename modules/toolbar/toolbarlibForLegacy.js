 ...
 *
 * Copyright (C) 2007 GidiJobs Labs, Inc.
 *
 *
 *
 *
...
cats_debug = function (msg)
{
    ...
    if (catsToolbarBranch.prefHasUserValue('debug'))
    {
        if (catsToolbarBranch.getCharPref('debug') == 1)
		{
			...
			consoleService.logStringMessage('GidiJobs: ' + msg);
		}
    }
    else
    {
...
cats_makeDefaultEnvironment = function()
{
    ...
    cats_toolbarAddButton(
        'Your version of the GidiJobs Toolbar is out of date.  Please click here to get the newest copy.',
        'Your version of the GidiJobs Toolbar is out of date.  Please click here to get the newest copy.',
...
/* Called whenever GidiJobs hits the logout button. */
...
cats_authenticationFailed = function(message)
{
    ...
    {
        cats_toolbarAddButton(
            'GidiJobs could not login: invalid username or password. Click here to configure the GidiJobs toolbar.',
            '',
            'cats_showOptionsDialog();',
            'cats-buttonLeftDisc',
            'cats_mainButton'
