...
switch ($action)
{
    ...
    case 'upgradeCats':
        ...
        /* This shouldn't be possible - there is no option to upgrade GidiJobs if no tables are in the database. */
        if (count($tables) == 0)
        {
    ...
    case 'maintComplete':
        ...
        file_put_contents(
            'INSTALL_BLOCK',
            'This file prevents the installer from running. Remove this file to edit or reset your GidiJobs installation.'
        );
...
function MySQLConnect()
{
    ...
    if (!$mySQLConnection)
    {
        die(
            ...
            . ' normal normal bold 12px/130% Helvetica Neue, Helvetica, sans-serif;">Error '
...
function MySQLQuery($query, $ignoreErrors = false)
{
    ...
    if (!$queryResult && !$ignoreErrors)
    {
        ...
        die (
            '<p style="background: #ec3737; padding: 4px; margin-top: 0; font:'
            . ' normal normal bold 12px/130% Helvetica Neue, Helvetica, sans-serif;">Query'
