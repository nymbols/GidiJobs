...
class SettingsUI extends UserInterface
{
    ...
    public function __construct()
    {
        ...
        /* Only GidiJobs professional on site gets to make career portal customizer users. */
    ...
    private function onAddNewTag()
    {
        ...
        {
            echo 'GidiJobs has lost your session data!';
            return;
        }
        ...
        switch ($action)
        {
           ...
           case 'loginActivity':
            ...
            case 'ajax_tags_add':
                if (!isset($_SESSION['CATS']) || empty($_SESSION['CATS']))
                {
                    echo 'App has lost your session data!';
                    return;
                }
                $this->onAddNewTag();
                break;
    ...
    private function newInstallPassword()
    {
        ...
        $this->_template->assign('prompt', 'Congratulations! You have successfully logged onto GidiJobs for the first time. Please create a new administrator password. Note that you cannot use \'cats\' as a password.');
    ...
    private function newSiteName()
    {
        ...
        $this->_template->assign('prompt', 'Your administrator password has been changed.<br /><br />Next, please create a name for your GidiJobs installation (for example, MyCompany, Inc.). This will be displayed in the top right corner of all GidiJobs pages.');
        $this->_template->assign('action', $this->getAction());
        $this->_template->assign('home', 'home');
        $this->_template->display('./modules/settings/NewInstallWizard.tpl');
    }
    ...
    private function upgradeSiteName()
    {
        ...
        $this->_template->assign('title', 'Site Name');
        $this->_template->assign('prompt', 'You have no site name defined. Please create a name for your GidiJobs installation (for example, MyCompany, Inc.). This will be displayed in the top right corner of all GidiJobs pages.');
        $this->_template->assign('action', $this->getAction());
        $this->_template->assign('home', 'home');
        $this->_template->display('./modules/settings/NewInstallWizard.tpl');
    }
    ...
    private function forceEmail()
    {
        ...
        $this->_template->assign('prompt', 'GidiJobs does not know what your e-mail address is for sending notifications. Please type your e-mail address in the box below.');
        $this->_template->assign('action', $this->getAction());
        $this->_template->assign('home', 'home');
        $this->_template->display('./modules/settings/NewInstallWizard.tpl');
    }
    ...
    private function onForceEmail()
    {
        ...
        else
        {
            ...
            $this->_template->assign('title', "E-Mail Address");
            $this->_template->assign('prompt', "Your e-mail settings have been saved. This concludes the GidiJobs initial configuration wizard.");
            $this->_template->assign('action', $this->getAction());
            $this->_template->assign('home', 'home');
            $this->_template->display('./modules/settings/NewInstallWizard.tpl');
        }
    ...
    private function newInstallFinished()
    {
        ...
        {
            $this->_template->assign('prompt', 'Your site name has been saved. This concludes the required GidiJobs configuration wizard.<BR><BR><span style="font-weight: bold;">Warning:</span><BR><BR> E-mail features are disabled. In order to enable e-mail features (such as e-mail notifications), please configure your e-mail settings by clicking on the Settings tab and then clicking on Administration.');
        }
        else
        {
            $this->_template->assign('prompt', 'Your site name has been saved. This concludes the required GidiJobs configuration wizard.');
        }
    ...
    private function onAspLocalization()
    {
        ...
        $this->_template->assign('prompt', 'Your localization settings have been saved. This concludes the GidiJobs initial configuration wizard.');
        $this->_template->assign('action', $this->getAction());
        $this->_template->assign('home', 'home');
        $this->_template->display('./modules/settings/NewInstallWizard.tpl');
    }
    ...
    private function manageProfessional()
    {
        ...
        if (isset($_GET['webFormPostBack']))
        {
            ...
            else if ($license->isProfessional())
            {
                ...
                {
                    $message = 'GidiJobs Professional requires the PHP SOAP library which isn\'t currently installed.<br /><br />'
                        ...
                        . 'Please visit http://www.gidijobs.co for more support options.';
                }
                ...
                {
                    $message = 'Internal Permissions Error<br /><span style="font-size: 12px; color: #000000;">GidiJobs is unable '
    ...
    private function wizard_checkKey()
    {
        ...
        if (isset($_GET[$id = 'key']) && $_GET[$id] != '')
        {
            ...
            if ($license->setKey($key) !== false)
            {
                if ($license->isProfessional())
                {
                    if (!CATSUtility::isSOAPEnabled())
                    {
                        echo "GidiJobs Professional requires the PHP SOAP library which isn't currently installed.\n\n"
                    ...
                    else
                    {
                        if (!LicenseUtility::validateProfessionalKey($key))
                        {
                            echo "That is not a valid Professional license key. Please visit "
                                . "us for more information about GidiJobs Professional.\n\n"
                                . "For a free open-source key, please visit and ";
                            return;
                        }
                    }
                }
        ...
        if ($fileError)
        {
            echo 'You entered a valid key, but this wizard is unable to write to your config.php file! You have '
                ...
                . 'For more help, visit our website at http://www.gidijobs.co for support options.';
        }

        echo 'That is not a valid key. You can register for a free open source license key on our website '
            . 'at http://www.gidijobs.co or a professional key to unlock all of the available features at '
            . 'http://www.gidijobs.co/professional';
    }
